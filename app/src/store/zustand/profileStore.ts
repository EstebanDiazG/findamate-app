...
interface ProfileState {
    ...
    profileUpdateDescription: (id: IProfile, description: {description: string}) => void;
...
export const profileStore = create<ProfileState>((set) => ({
    ...
    profileUpdateDescription: async (id, description) => {
        try {
            set({ isLoading: true });
            const response = await apiInstance.put(`/profile/id/${id}`, { description });
            const { data } = response.data;
            set({ profile: data, isLoading: false });
        } catch (error) {
            set({ isError: true, error: (error as Error).message, isLoading: false });
